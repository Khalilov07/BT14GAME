...
let timer; // в ней не чего нет

$("#btn-start").on("click", function () {

  // записываем setInterval в переменную timer
  timer = setInterval(function () {
    if (time >= 0) {
      $("#text").text(`time: ${time--}`); // мы находим h1 и меняем контент h1
    } else {
      $("#finish-window").css("display", "flex");
      $("#finish-window h2").text("ТЫ ЧУШПАН!");
    }
...
$("#box").on("click", function () {
  ...
  // зачем нужен ToggleClass()

  if (count > 10) {
    clearInterval(timer); // остановка таймера
    $("#finish-window").css("display", "flex");
    $("#finish-window h2").text("ТЫ МУЖЧИНА!");
  } else {
